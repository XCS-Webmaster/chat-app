document.addEventListener("DOMContentLoaded", () => {
  ...
  // The element always shows the avatar first, then the bubble.
  // If a fileURL is provided:
  //  • If it starts with "data:image/", display its thumbnail with "View" and "Download" buttons below it.
  ...
  function buildMessageElement(who, text, isCustomer, fileURL = null) {
    ...
    if (fileURL) {
      if (fileURL.startsWith("data:image/")) {
        // Display image thumbnail with "View" and "Download" buttons using the common .btn style.
        bubble.innerHTML = `
  ...
  function updateVisitorTab(visitorId) {
    ...
    if (!btn) {
      ...
      btn.onclick = () => {
        ...
        // Clear alert flag so new first-message alert can trigger.
  ...
  socket.on("chat message", (msg) => {
    // Only process customer messages (ignore if msg.from is undefined or equals "support").
